...
> O projeto originalmente também previa uma segunda frente — indícios de
> autoria por pessoa jurídica no desenvolvimento dos sites, cruzada com
> a prestação de contas — que foi **deixada fora do escopo** desta
> rodada. O código correspondente permanece no repositório para eventual
> retomada futura, mas não é reportado aqui.
...
públicos disponíveis (ver seção 5.2 de `METODOLOGIA.md`). Uma checagem
...
- A coleta foi feita em 15/09/2026, um retrato pontual no tempo — sites
  continuam sendo criados/alterados/desativados ao longo da campanha
...
- Ver seção 7 de `METODOLOGIA.md` para a lista completa de limitações.

## 5. Reprodutibilidade

Código-fonte completo, dados brutos e processados disponíveis em
repositório privado no GitHub (`raschmitt/auditoria-sites-candidatos-2026`).
